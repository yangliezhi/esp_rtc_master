#include <stdio.h>
#include "du_bsp_pwm_motor.h"

static const char *TAG = "du_bsp_pwm_motor";

#define BDC_PID_EXPECT_SPEED 10 // expected motor speed, in the pulses counted by the rotary encoder
typedef struct
{
...
static void pid_loop_cb(void *args)
{
    ...
    // ESP_LOGI(TAG, "pid_cpmpute receive new_speed cur_pulse_count is %d", cur_pulse_count);
    ...
    float new_speed = 0;
    // ESP_LOGI(TAG, "pid_cpmpute receive new_speed error is %f", error);
    // set the new speed
    pid_compute(pid_ctrl, error, &new_speed);
    // ESP_LOGI(TAG, "pid_cpmpute receive new_speed is %f", new_speed);

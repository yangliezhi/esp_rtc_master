/*
 * @Author: 都建民
 * @Date: 2024-03-20 02:17:03
 * @Last Modified by: dujianmin qq群 26875961
 * @Last Modified time: 2024-03-21 14:15:38
 */
/*
...
void wifi_task(void *pvParameters)
{
    ...
    while (1)
    {
        EventBits_t bits = xEventGroupWaitBits(global_event_group, WIFI_CONNECTED_BIT_G | WIFI_STOP_BIT_G | WIFI_STANDBY_BIT_G, pdTRUE, pdFALSE, portMAX_DELAY);

        if (bits & WIFI_CONNECTED_BIT_G)
        {

            enable_wifi(true);                                                  // 打开 WiFi
            xEventGroupClearBits(global_event_group, WIFI_CONNECTED_BIT_G); // 处理完事件后清除标志位
        }
        else if (bits & WIFI_STOP_BIT_G)
        {
            disable_wifi(true);                                            // 关闭 WiFi
            xEventGroupClearBits(global_event_group, WIFI_STOP_BIT_G); // 处理完事件后清除标志位
        }
        else if (bits & WIFI_STANDBY_BIT_G)
        {
            // 进入待机状态，可以在这里添加相应的操作
            xEventGroupClearBits(global_event_group, WIFI_STANDBY_BIT_G); // 处理完事件后清除标志位
        }
        else
        {
            // 处理未匹配到任何标志位的情况

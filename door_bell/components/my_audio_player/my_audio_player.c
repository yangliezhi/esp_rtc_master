...
i2c_bus_handle_t i2c_bus_create_1(i2c_port_t port, i2c_config_t *conf)
{
    ...
    ret = i2c_driver_install(i2c_bus[port]->i2c_port, i2c_bus[port]->i2c_conf.mode, 0, 0, 0);
    if (ret != ESP_OK)
    {
        goto error;
    }
...
static esp_err_t bsp_i2c_init_1(void)
{
    ...
    // BSP_ERROR_CHECK_RETURN_ERR(i2c_param_config(0, &i2c_conf));
    // BSP_ERROR_CHECK_RETURN_ERR(i2c_driver_install(0, i2c_conf.mode, 0, 0, 0));
    i2c_bus_create_1(0, &i2c_conf);
...
static esp_err_t bsp_i2c_device_probe(uint8_t addr)
{
    ...
    i2c_master_stop(cmd);
    if (i2c_master_cmd_begin(0, cmd, 1000) == ESP_OK)
    {
        ret = ESP_OK;
    }
...
{
    if (ESP_OK == bsp_i2c_device_probe(esp_io_expander_new_i2c_tca9554(0, ESP_IO_EXPANDER_I2C_TCA9554_ADDRESS_000, &io_expander)))
    {
    ...
    else
    {
        ...
        esp_io_expander_new_i2c_tca9554(0, ESP_IO_EXPANDER_I2C_TCA9554A_ADDRESS_000, &io_expander);
    }
